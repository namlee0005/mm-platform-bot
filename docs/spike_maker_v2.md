...
1. **VWAP Anchor**: `mid = VWAP_3m` (hoặc `VWAP_rolling`). Bắt bài lực mua/bán thực tế thay vì chỉ nhìn Bid/Ask ảo.
2. **Fair Price (Shifted)**:
   ```
   fairPrice = mid (VWAP)
            + 0.7 × (microPrice - mid) 
            + 0.1 × OFI_tilt 
            + (Momentum × 5.0) 
   ```
   Khi sử dụng VWAP làm Anchor, `fairPrice` phản ánh đúng "giá trị thực" của dòng tiền. Khi giá bị đẩy, Momentum cộng hưởng với VWAP tăng sẽ đẩy Fair Price lên cực nhanh.
3. **Spike Score (S)**: `|ret| / sigma`. Đo lường độ "spike" của giá. Giới hạn ở 10.
4. **Spread Scaling (Hard Floor)**:
   `halfSpread = max(spread_min_bps, 30bps × (1 + 2 × S^0.7))`
   **CRITICAL**: Tuyệt đối không để spread sập xuống mức TickSize như trong log.
5. **Ladder Distances**: `distances[i] = (halfSpread) + (maxDist - halfSpread) × (i/(n-1))^gamma`.
6. **Size Profile**: `weight = 1.3 ^ i`. Mỏng ở L0, dày ở L10 (đưa size về 1.0 nếu S > 3).
7. **Inventory Skew & Price Displacement**:
   - **Size Skew**: `sizeMultBid = max(0.2, 1 - 0.8 × skew)`
   - **Price Shift**: `invPriceShift = skew × 2.0%` 
   Khi `invRatio` âm (bị gom hàng), bot tự động đẩy giá chào bán (Ask) lên **+2.0%** để cắt đứt đà tích lũy của Attacker.
8. **Asymmetric Defense (Anti-Dump Logic)**: 
   Nếu giá vừa tăng đột ngột (`returns > 0`):
   - **Tăng cường Spread BID**: `bidDist *= (1 + 0.5 × S)`. Giãn lệnh MUA ra thật xa để không bị Attacker "xả" hàng vào đầu ở đỉnh Pump.
   - **Giữ nguyên/Nới nhẹ ASK**: Không cần nới ASK quá mạnh vì Attacker không còn nhu cầu mua thêm ở giá cao.
   *Ngược lại, nếu giá giảm mạnh (Dump) → Giãn mạnh ASK để cản đà xả, giữ BID an toàn.*
9. **Ninja Protection**: 
